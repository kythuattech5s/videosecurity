...
<table id="order_pcb" style="width:100%">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{HaviHelper::$TYPE_BOARDS[$dataItem->type_board][1]}}</td>

      <td><strong>Phản hồi</strong></td>

      <td colspan="3">

        <textarea name="feedback" style="width: 100%;height: 150px;">{{$dataItem->feedback}}</textarea>
    ...
    <tr>
      ...
      <td>{{$dataItem->length}}x{{$dataItem->width}} mm</td>
      ...
      <td>{{HaviHelper::$LAYERS[$dataItem->layer][1]}}</td>
    ...
    <tr>
      ...
      <td>{{HaviHelper::$MATERIALS[$dataItem->material][1]}}</td>

      <td><strong>Độ d&agrave;y</strong></td>

      <td>{{HaviHelper::$THICHNESS[$dataItem->thichness][1]}}</td>

      <td><strong>Min Track /spacing</strong></td>

      <td>{{HaviHelper::$MIN_TRACKS[$dataItem->min_track][1]}}</td>

    </tr>

    <tr>

      <td><strong>Min hole size</strong></td>

      <td>{{HaviHelper::$MIN_HOLE_SIZES[$dataItem->min_hole][1]}}</td>

      <td><strong>M&agrave;u sơn</strong></td>

      <td>{{HaviHelper::$COLORS[$dataItem->color][1]}}</td>

      <td><strong>Silk screen</strong></td>

      <td>{{HaviHelper::$SILK_SCREENS[$dataItem->silk_screen][1]}}</td>
    ...
    <tr>
      ...
      <td>{{HaviHelper::$SURFACE_FINISHS[$dataItem->surface_finish][1]}}</td>

      <td><strong>Độ d&agrave;y lớp đồng</strong></td>

      <td>{{HaviHelper::$FINISHED_COPPERS[$dataItem->finish_coppers][1]}}</td>
    ...
    <tr>
      ...
      <td colspan="3"><strong><i>{{$dataItem->content}}</i></strong></td>
    ...
    <tr>
      ...
      <td> <strong style="color:#d29925">
        <input type="text" class="total_money" value="{{number_format($dataItem->total_money)}}">
        <input class="hidden" type="number" name="total_money" value="{{$dataItem->total_money}}"> đ</strong>

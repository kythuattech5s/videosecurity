...
<div class="navigation" data-menu = "<?php echo session('menu_status', 'off') ?>">
  ...
  <ul class="main-menu">
    ...
    <li class="nav-item"><a href="{{FCHelper::ep($pmenu,'link')}}"><i class="{{$pmenu->icon}}"></i><span style="<?php session("menu_status",'off')=='on'?'display:inline-block;height:inherit;width:inherit;':'display:block;height:0px;width:0px'; ?>" class="txt">{{FCHelper::ep($pmenu,'name')}}</span></a>
      <ul class="sub none" >
